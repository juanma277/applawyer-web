  ...
  constructor( public http: HttpClient,
   ...
   loginGoogle(token: string){
     ...
     return this.http.post(url, {token})
                .map((resp:any)=>{
                  this.guardarStorage(resp.id, resp.usuario, resp.token.token);
                  return true;
                });
   }
    ...
    if(recordar === true){
      ...
      return this.http.post(url, {'email': usuario.email, 'password': usuario.password} )
                  ...
                  .map((resp:any) =>{
                    ...
                    this.guardarStorage(resp.usuario.id, resp.usuario,  resp.token.token);
                    return true;
                  })
                  .catch(err =>{
   ...
   crearUsuario( usuario: Usuario ){
     ...
     return this.http.post(url, {
                                   ...
                                   'provider': 'Normal',
                                   'uid': null,
                                   'notificaciones': usuario.notificaciaones, 
                                   'terminos': usuario.condiciones
                          })
                .map((resp: any) => {
                  if(resp.error){
   ...
   actualizarUsuario(usuario: Usuario){
    ...
    let url = URL_SERVICIOS + '/users/' + usuario._id;
    url += '?token=' + this.token;
